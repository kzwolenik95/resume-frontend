...
function incrementCounter(callback) {
  ...
  request.open('POST', 'https://4wgie6nlp1.execute-api.us-east-1.amazonaws.com/Prod/increment');
...
function getCounterValue(callback) {
  if (!getCookie('counter_value')) {
    ...
    request.open('GET', 'https://4wgie6nlp1.execute-api.us-east-1.amazonaws.com/Prod/increment');
    request.send();
  } else {
    callback();
  }
